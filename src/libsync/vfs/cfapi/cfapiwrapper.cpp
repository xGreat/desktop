...
{
    if (path.isEmpty()) {
        qCError(lcCfApiWrapper) << "Empty path" << path;
        return {};
    }

    QFileInfo pathFileInfo(path);
    if (!pathFileInfo.exists()) {
        qCError(lcCfApiWrapper) << "Path does not exists" << path;
        return {};
    }
    ...
    if (pathFileInfo.isDir()) {
        ...
        qCError(lcCfApiWrapper) << "CfOpenFileWithOplock fails with path" << path << "with error:" << GetLastError();
    ...
    qCError(lcCfApiWrapper) << path << "is not a file and not a directory";
